...
class Pages {
    ...
    public $config  =    array('first'=>'首页','prev'=>'上一页','theme'=>' %first% %upPage% %linkPage%  %nextPage% %downPage%  %prePage% %end% %jilu% ','next'=>'下一页','last'=>'尾页','header'=>'条记录');
    // 默认分页变量名
    protected $varPage;
...
//<li><a href='#'><img src='/Home/Tpl/Public/images/next.jpg' width='21' height='21'></a></li>
        ...
        // if($nowCoolPage == $this->coolPages){
			...
			// $theEndRow  =   $this->totalPages;
			// $theEnd     =   "<a href='".str_replace('__PAGE__',$theEndRow,$url)."' >".$this->config['last']."</a></li>";
        // }else{
            // $nextRow    =   $this->nowPage+$this->rollPage;
            // $theEndRow  =   $this->totalPages;
		...
		//$jilu='<span>'.$nowPage.'/'.$totalPages.'页     共'.$totalRows.'条记录     '.$listRows.'记录每页</span>';
        $pageStr     =   str_replace(
