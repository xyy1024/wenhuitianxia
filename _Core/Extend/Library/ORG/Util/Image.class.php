...
class Image {
	...
	/**
	 * 生成特定尺寸的缩略图 PS：按原尺寸的一条边为基准等比例压缩图片
     * @static
     * @access public
     * @param string $image  原图
     * @param string $type 图像格式
     * @param string $thumbname 缩略图文件名
     * @param string $both  0:宽,1:度
     * @param string $maxw  高度
     * @param boolean $interlace 启用隔行扫描
     * @return void
     */
    static function thumb3($image, $thumbname, $type='', $both=0, $maxw=50, $interlace=true) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
            ...
            unset($info);
			if($both==1){
				$scale = $maxw / $srcHeight; // 计算缩放比例
			}else{
				$scale = $maxw / $srcWidth; // 计算缩放比例
			}
            if ($scale >= 1) {
                // 超过原图大小不再缩略
                $width = $srcWidth;
