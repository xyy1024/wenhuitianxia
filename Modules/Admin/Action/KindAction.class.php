...
class KindAction extends Action{
	...
	public function upload_json(){
		...
		$ext_arr = array(
			...
			'file' => array('doc', 'docx', 'xls', 'xlsx', 'ppt', 'htm', 'html', 'txt', 'zip', 'pdf', 'rar', 'gz', 'bz2', 'flv'),
		);
		//最大文件大小
		$max_size = 100000000;
		...
		if (empty($_FILES) === false) {
			...
			//将附件的路径存入attachments表中
			$data_file['file'] =  $file_url;
			$data_file['addtime'] =  time();
			M('Attachments')->add($data_file);
			

			header('Content-type: text/html; charset=UTF-8');
			$json = new Services_JSON();
			echo $json->encode(array('error' => 0, 'url' => $file_url, 'file_name' => $file_name));
			exit;
		}
	}
