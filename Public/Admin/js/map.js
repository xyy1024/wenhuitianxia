function baidu_map(data)
{
	...
	var marker1;
	var marker2;
	map.enableScrollWheelZoom();	//滚轮放缩
	if (typeof(data.lng) != 'undefined' && typeof(data.lat) != 'undefined')
	{
		...
		var infoWindow = new BMap.InfoWindow("原本位置 " + data.adr + " ,移动红点修改位置!你也可以直接修改地图标识系统自动定位!", opts);  // 创建信息窗口对象
	...
	function auto()
	{
		...
		geolocation.getCurrentPosition(function (r)
		{
			if (this.getStatus() == BMAP_STATUS_SUCCESS)
			{
				...
				var infoWindow = new BMap.InfoWindow("定位成功这是你当前的位置!,移动红点标注目标位置，你也可以直接修改地图标识,系统自动定位!", opts);  // 创建信息窗口对象
				marker1.openInfoWindow(infoWindow);      // 打开信息窗口
				doit(point);
